...
void SlamManager::run()
{
    ...
    while( !bexit_required_ ) {
        ...
        if( getNewImage(img_left, img_right, time) )
        {
            ...
            if( frame_id_ > 0 ) {
                cam_delay = rclcpp::Time::now().toSec() - last_img_time;
                last_img_time += cam_delay;
            } else {
                last_img_time = rclcpp::Time::now().toSec();
            }
            ...
            // ========================================
            bool c1 = cam_delay > 0;
            bool c2 = ( rclcpp::Time::now().toSec() - last_img_time ) > 100. * cam_delay;
...
void SlamManager::writeResults()
{
    ...
    if( pslamstate_->do_full_ba_ ) 
    {
        ...
        prosviz_->pubPointCloud(pmap_->pcloud_, rclcpp::Time::now().toSec());
